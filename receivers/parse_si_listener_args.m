function [si_root, mode] = parse_si_listener_args(varargin)
%PARSE_SI_LISTENER_ARGS Split start_si_listener's arguments into root + mode.
%   [si_root, mode] = PARSE_SI_LISTENER_ARGS(...)
%
%   The tiff root stays POSITIONAL so start_si_listener('E:') keeps meaning what it
%   always did, and 'blocking' / 'async' are recognised as keywords in any position.
%   Matching is case-insensitive and tolerates surrounding whitespace.
%
%       ()                  -> si_root = '',   mode = 'async'
%       ('E:')              -> si_root = 'E:', mode = 'async'
%       ('blocking')        -> si_root = '',   mode = 'blocking'
%       ('E:', 'blocking')  -> si_root = 'E:', mode = 'blocking'
%
...
%   See also: start_si_listener, test_si_listener_args
    ...
    assert(numel(args) <= 1, 'start_si_listener:tooManyArgs', ...
        ['Expected at most a tiff root plus ''blocking''/''async''; got %d other ' ...
         'argument(s). Usage: start_si_listener(''E:'', ''blocking'')'], numel(args));
