...
pulseDuration = 10; %ms Stimulation pulse 
TrigDuration = 5; %ms SLM flip command
stimFreq= 1; % Shouldn't matter but is used

% P1= [.01 : .01 : .07]; %[.01 .02 .03 .04 .050 .075];%[.05 .06 .07 .08 .09 .1];%[.01 .025 .04 .055 .07 .085 0.1 0.115 0.13]; %[0.05 0.1 0.125 0.15];%[.01 .02 .03 .04 .05];%[0.05 .1 .15];%List of Powers To Use
% P1 = [20:10:150]/1000;
% P1 = [12.5 25:25:200]/1000;
% P1 = [25 50 75 100 125 150]/1000;
% P1 = [25:12.5:100]/1000;
% P1 = [ .05 .075 .1 .125];
P1 = [0.005];
% P1 = [25:25:125]/1000;
...
% makeHoloTrigSeqs2K(Seq, holoRequest, slm, laser_eom);
